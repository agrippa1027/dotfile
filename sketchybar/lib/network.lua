...
function network.interface(hardware_ports)
	...
	for line in tostring(hardware_ports or ""):gmatch("[^\r\n]+") do
		...
		if port then
			is_wifi = port == "Wi-Fi" or port == "AirPort"
		end
		local device = line:match("^Device:%s*([%w]+)")
		if is_wifi and device then
			return device
		end
	end
end
...
function network.parse(data)
	...
	local ip = utils.trim(data.ip)
	if not ip:match("^%d+%.%d+%.%d+%.%d+$") or ip == "0.0.0.0" then
		ip = nil
	end
	local ssid = utils.trim(summary:match("\n%s*SSID%s*:%s*([^\r\n]+)"))
	if ssid == "" or ssid:lower() == "<redacted>" or ssid == "(null)" then
		ssid = nil
	end
