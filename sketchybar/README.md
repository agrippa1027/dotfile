...
Lua configuration using the official SbarLua API. Hubbamax colours and Comic Code labels, with compact coloured sections and a powerline app indicator inspired by neutonfoo's layout. Community icons and current FelixKratz volume/battery/calendar modules are reused; pinned sources and adaptations are documented in [vendor/README.md](vendor/README.md).
...
- Apple icon or current app opens the settings popup. It also provides Bluetooth settings.
...
- **Alt-M** explicitly alternates between native app menus and SketchyBar. Press again to return. Icon clicks keep the bar visible.
...
The full-width opaque bar uses `topmost=on` to cover the auto-hidden native menu bar during normal use. AeroSpace reserves a 48-point top gap. To disable SketchyBar, stop its Homebrew service, restore `outer.top = 8` in AeroSpace, and re-enable the native menu bar.
