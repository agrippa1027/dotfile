...
local function refresh()
  ...
  sbar.exec("/opt/homebrew/bin/aerospace list-workspaces --focused", function(focused, status)
    if status ~= 0 or current ~= revision then return end
    focused = utils.trim(focused)
    sbar.exec("/opt/homebrew/bin/aerospace list-windows --all --format '%{workspace}%{tab}%{app-name}' --json", function(windows, result)
      if result ~= 0 or type(windows) ~= "table" or current ~= revision then return end
      local apps = {}
      ...
      for workspace, item in pairs(workspaces) do
        local labels = {}
        for app in pairs(apps[workspace] or {}) do labels[#labels + 1] = app_icons[app] or ":default:" end
        table.sort(labels)
        ...
        item:set({
          ...
          label = { string = table.concat(labels, " "), drawing = #labels > 0, color = active and colors.black or colors.white },
...
state:subscribe({ "forced", "aerospace_workspace_change", "front_app_switched", "space_windows_change", "system_woke" }, refresh)
refresh()
